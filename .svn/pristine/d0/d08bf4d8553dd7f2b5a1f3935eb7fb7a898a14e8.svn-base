	...
	$(function() {
		...
		$("#btn_edit").click(function() {
			...
			if(choice == 0) {
				...
				alert("수정할 타인 및 타인 입력 지정자를 선택해 주세요.");
			} else if(choice > 1) {
				
				document.getElementById("PeopleAddTable").style.display = "none";
				alert("수정할 타인 및 타인 입력 지정자를 하나만 선택해주세요.");
			} else {
		...
		$("#btn_delete").click(function() {
			...
			if(choice == 0) {
				...
				alert("제거할 타인 및 타인 입력 지정자를 선택해 주세요.");
			} else {
				
				if(false!=confirm("선택한 타인 및 타인 입력 지정자를 제거하시겠습니까?")){
...
<body>
	<form id="userInfo" name="userInfo" method="post">
		...
		<div class="mm-menu-name"><h5>관리자 M/M (타인 입력 지정)</h5></div>
					...
					<table id="peopleInserEdit" name="peopleInserEdit" class="table table-bordered">
						<thead>
							<tr class="table-primary">
								<th rowspan="2" scope="col" style="color: #848484; text-align:center; vertical-align:middle; width: 200px;">작업 년도</th>
			...
			<tr>
				<td>
					<table class="table table-bordered">
						<thead>
							<tr class="table-primary">
								<th rowspan="2" scope="col" width=90px style="color: #848484; text-align:center; vertical-align:middle;"><input type="checkbox" id="allCheckOther" name="allCheckOther" onclick="allOther()"/>  선택</th>
