...
<body>
	<form id="userInfo" name="userInfo" method="post">
		...
		<table id="project_code_table" name="project_code_table" style="text-align: center; margin-left: auto; margin-right: auto; border:1px solid #444444;">
			<thead>
				<tr class="table-primary">
					<th scope="col" style="color: #848484; width:70px">선택</th>
					<th scope="col" style="color: #848484; width:400px">프로젝트 명</th>
					<th scope="col" style="color: #848484; width:100px">투입 M/M</th>
				</tr>
			</thead>
			<c:forEach var='item' items="${lUserDefaultMM }">
				<c:if test="${item.v_project_name ne null }">
					<tr id="trName" name="trName">
						<td><input type='checkbox' id='code_check' name='code_check' value="${item.i_project_code_pk }" /></td>
						<td><input type="text" id="code_name" name="code_name" value="${item.v_project_name }" size="45" readonly style="border: 0;" /></td>
						<td><input type="text" id="code_mm" name="code_mm" value="${item.d_mm_user }" size="5" readonly style="border: 0;" /></td>
						<td><input type='text' id='code_number' name='code_number' value="${item.i_project_code_pk }" style='display: none;' /></td>
					</tr>
				</c:if>
			</c:forEach>
		</table>
		...
		<table style="margin-left: auto; margin-right: auto;">
			<tr>
				<th>
					<input type="button" class="btn btn-primary btn-sm" id="btn_save" name="btn_save" value="저장" style="width:60px; align: right;" />
				</th>
				<th>
					<input type="button" class="btn btn-primary btn-sm" id="btn_delete" name="btn_delete" value="삭제" style="width:60px; align: right;" />
				</th>
			</tr>
		</table>
	</form>
