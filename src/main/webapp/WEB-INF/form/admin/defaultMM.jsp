...
$(function() {
	...
	$("#btn_add").click(function(){
		...
		cell1.innerHTML = "<td><select id='addDefaultYear' name='addDefaultYear' class='form-control' style='width: 120px;'><option value=''>작업월</option><c:forEach var='item' items='${lMonth }'><option value='${item }'>${item }</option></c:forEach></select></td>";
		...
		cell4.innerHTML = "<td><select id='addDefaultDept' name='addDefaultDept' style='width: 240px;'><option value=''></option><c:forEach var='item' items='${lDept }'><option value='${item.i_seq_pk }'>${item.v_up_dept_name_k } / ${item.v_dept_name_k }</option></c:forEach></select></td>";
		...
		cell7.innerHTML = "<td><input type='text' id='addDefaultIn' name='addDefaultIn' style='width: 100px;' /></td>";
		cell8.innerHTML = "<td><input type='text' id='addDefaultOut' name='addDefaultOut' style='width: 100px;' /></td>";
		cell9.innerHTML = "<td><input type='text' id='addDefaultDay' name='addDefaultDay' style='width: 100px;' /></td>";
		cell10.innerHTML = "<td><input type='text' id='addDefaultMM' name='addDefaultMM' style='width: 100px;'  /></td>";
	...
	$("#btn_edit").click(function(){
		...
		if(choice == 1) {
			...
			cell1.innerHTML = "<td><input type='hidden'	id='addProjectSeq'	name='addProjectSeq' value='" + defaultSeq + "' /><input type='text' id='addDefaultYear' name='addDefaultYear' readonly style='border: 0; width: 120px;' value='" + defaultYear + "' /></td>";
			...
			cell6.innerHTML = "<td><select id='addDefaultName' name='addDefaultName' onchange='changeUser()'><option value=''></option><c:forEach var='item' items='${lUser }'><option value='${item.userSEQ }'>${item.userName }</option></c:forEach></select></td>";
			cell7.innerHTML = "<td><input type='text' id='addDefaultIn' name='addDefaultIn' style='width: 100px;' value='" + defaultIndate + "' /></td>";
			cell8.innerHTML = "<td><input type='text' id='addDefaultOut' name='addDefaultOut' style='width: 100px;' value='" + defaultOutdate + "' /></td>";
			cell9.innerHTML = "<td><input type='text' id='addDefaultDay' name='addDefaultDay' style='width: 100px;' value='" + defaultDay + "' /></td>";
			cell10.innerHTML = "<td><input type='text' id='addDefaultMM' name='addDefaultMM' style='width: 100px;' value='" + defaultMM + "' /></td>";
		...
		<tr>
			<td>
				<table class="table table-bordered">
					<thead>
						<tr class="table-primary">
							<th scope="col" style="color: #050505; text-align: center; font-size: 14px;"><input type="checkbox" id="allCheckDefault" name="allCheckDefault" onclick="allProject()"/>  선택</th>
							<th scope="col" style="color: #050505; font-size: 14px;">작업년도</th>
							<th scope="col" style="color: #050505; font-size: 14px;">부서코드</th>
							<th scope="col" style="color: #050505; font-size: 14px;">부서명(ERP)</th>
							<th scope="col" style="color: #050505; font-size: 14px;">부서명</th>
							...
							<th scope="col" style="color: #050505; font-size: 14px;">입사일</th>
							<th scope="col" style="color: #050505; font-size: 14px;">퇴사일</th>
							<th scope="col" style="color: #050505; font-size: 14px;">급여 일수</th>
							<th scope="col" style="color: #050505; font-size: 14px;">당월 M/M</th>
						</tr>
					</thead>
					<c:if test="${lDefaultMM.size() != 0 }">
						<c:forEach var="item" items="${lDefaultMM }">
							<tr>
								<td style="text-align: center;vfont-size: 14px;">
								...
								<td id="DefaultDate" name="DefaultDate" style="font-size: 14px;">${item.d_job_date }</td>
								<td id="DefaultDeptCode" name="DefaultDeptCode" style="font-size: 14px;">${item.v_dept_code }</td>
								<td id="DefaultDeptERP" name="DefaultDeptERP" style="font-size: 14px;">${item.v_dept_code_name }</td>
								<td id="DefaultDeptKor" name="DefaultDeptKor" style="font-size: 14px;">${item.v_dept_name }</td>
								<td id="DefaultMMNum" name="DefaultMMNum" style="font-size: 14px;">${item.v_man_seq }</td>
								<td id="DefaultMMName" name="DefaultMMName" style="font-size: 14px;">${item.v_user_name_k }</td>
								<td id="DefaultMMIndate" name="DefaultMMIndate" style="font-size: 14px;">${item.d_enter_date }</td>
								<td id="DefaultMMOutdate" name="DefaultMMOutdate" style="font-size: 14px;">${item.d_retirement_date }</td>
								<td id="DefaultMMDay" name="DefaultMMDay" style="font-size: 14px;">${item.i_day_count }</td>
								<td id="DefaultMMNow" name="DefaultMMNow" style="font-size: 14px;">${item.d_mm }</td>
							</tr>
						</c:forEach>
					</c:if>
					<c:if test="${lDefaultMM.size() != 0 }">
						<c:forEach var="item" items="${lDefaultMM }" begin="0" end="0">
							<tr>
								...
								<td id="totalMM" name="totalMM" style="font-size: 14px;">${item.d_mm_sum }</td>
							</tr>
						</c:forEach>
					</c:if>
				</table>
