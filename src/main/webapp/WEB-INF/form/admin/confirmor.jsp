		...
		if($("#search_date").val() == "") {
			...
			$("[name='checkConfirmor']").each(function(){
				...
				if($(this).is(":checked")){
					...
					confirmorSeq += $(this).val() + ","
				}
			});
				
			$("#confirmorSeq").val(confirmorSeq);
			$("#urlPage").val("confirmor");
	...
	<table class="mm-search">
		<tr>
			<td style="margin-top:10px;">
				<button type="button" id="confirmorDown" name="confirmorDown" class="btn btn-outline-primary" >Excel 다운로드</button>
		...
		<tr>
			<td>
				<table class="table table-bordered">
					<thead>
						<tr class="table-primary">
							<th rowspan="2" scope="col" width=90px style="color: #050505; text-align:center; vertical-align:middle; font-size: 14px;"><input type="checkbox" id="allCheckConfirmor" name="allCheckConfirmor" onclick="allConfirmor()"/>  선택</th>
							<th rowspan="2" scope="col" width=100px style="color: #050505; text-align:center; vertical-align:middle; font-size: 14px;">작업년도</th>
							<th colspan="3" scope="col" style="color: #050505; text-align:center; font-size: 14px;">확인자</th>
						</tr>
						<tr class="table-primary">
							<th scope="col" style="color: #050505; text-align:center; font-size: 14px;">사번</th>
							<th scope="col" style="color: #050505; text-align:center; font-size: 14px;">성명</th>
							<th scope="col" width=150px style="color: #050505; text-align:center; font-size: 14px;">팀 마감여부</th>
						</tr>
					</thead>
					<c:if test="${lConfirmor.size() != 0 }">
						<c:forEach var="item" items="${lConfirmor }">
							<tr>
								<td style="text-align: center; font-size: 14px;"><input type="checkbox" id="checkConfirmor" name="checkConfirmor" value="${item.i_seq_pk }"/></td>
								...
								<td id="confirmorNu" name="confirmorNu" style="font-size: 14px;">${item.v_man_seq }</td>
								<td id="confirmorNa" name="confirmorNa" style="font-size: 14px;">${item.v_user_name_k }</td>
								<td id="confirmorCimplete" name="confirmorCimplete" style="font-size: 14px;">${item.v_complete }</td>
							</tr>
						</c:forEach>
					</c:if>
				</table>
